...
p = var.sel(depth=slice(0,1000)).plot.contourf(ax=ax, cmap=colors,vmin=-val, vmax=val, extend='both', levels=31, cbar_kwargs={'drawedges': True, 'shrink' : 1.})
mask.sel(depth=slice(0,1000)).plot.contour(ax=ax, levels=1, colors='k')
ax.set_xlabel(xname)
ax.invert_yaxis()
ax2 = fig.add_subplot(212)
p2 = var.plot.contourf(ax=ax2, cmap=colors, vmin=-val, vmax=val, extend='both', levels=31, cbar_kwargs={'drawedges': True, 'shrink' : 1.})
